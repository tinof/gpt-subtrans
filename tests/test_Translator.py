...
class SubtitleTranslatorTests(SubtitleTestCase):
    ...
    def test_SubtitleTranslator(self):
        ...
        for data in test_data:
            ...
            translator.events.batch_translated += lambda batch: self.validate_batch(
                batch, original=originals, reference=reference
            )
            translator.events.scene_translated += lambda scene: self.validate_scene(
                scene, original=originals, reference=reference
            )
    ...
    def test_PostProcessTranslation(self):
        ...
        for data in test_data:
            ...
            differences = sum(
                1 if reference.originals[i] != originals.translated[i] else 0 for i in range(len(originals.originals))
            )
            unchanged = sum(
                1 if reference.originals[i] == originals.translated[i] else 0 for i in range(len(originals.originals))
            )

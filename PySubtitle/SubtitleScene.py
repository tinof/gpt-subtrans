...
class SubtitleScene:
    ...
    def AddNewBatch(self):
        batch = SubtitleBatch({"scene": self.number, "number": len(self.batches) + 1})
        self._batches.append(batch)
    ...
    def SplitBatch(self, batch_number: int, line_number: int, translated_number: int = None):
        ...
        split_index = next(
            (i for i, line in enumerate(batch.originals) if line.number == line_number),
            -1,
        )
        if split_index <= 0:
        ...
        if batch.translated:
            ...
            translated_index = next(
                (i for i, line in enumerate(batch.translated) if line.number == split_translated),
                -1,
            )

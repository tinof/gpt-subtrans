...
class SubtitleFile:
    ...
    def GetBatchesContainingLines(self, line_numbers: list[int]):
        ...
        for scene in self.scenes:
            ...
            for batch in scene.batches:
                ...
                while (
                    next_line_index < line_number_count and last_line_in_batch >= sorted_line_numbers[next_line_index]
                ):
                    next_line_index += 1
    ...
    def LoadSubtitles(self, filepath: str = None):
        ...
        try:
            with open(self.sourcepath, encoding=default_encoding, newline="") as f:
                source = list(srt.parse(f))

        except srt.SRTParseError:
            with open(self.sourcepath, encoding=fallback_encoding) as f:
                source = list(srt.parse(f))
    ...
    def UpdateLineText(self, line_number: int, original_text: str, translated_text: str):
        with self.lock:
            original_line = next(
                (original for original in self.originals if original.number == line_number),
                None,
            )
            ...
            translated_line = (
                next(
                    (translated for translated in self.translated if translated.number == line_number),
                    None,
                )
                if self.translated
                else None
            ...
            translated_line = SubtitleLine.Construct(
                line_number, original_line.start, original_line.end, translated_text
            )
            ...
            for scene in self.scenes:
                ...
                for batch in scene.batches:
                    ...
                    unmatched_translated = [
                        line for line in batch.translated if line.number not in original_line_numbers
                    ]
                    if unmatched_translated:
                        logging.warning(
                            f"Removing {len(unmatched_translated)} translations lines in batch ({batch.scene},{batch.number}) that don't match an original line"
                        )

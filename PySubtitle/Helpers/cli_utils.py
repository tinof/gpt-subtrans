...
@dataclass
class LoggerOptions:
    file_handler: logging.FileHandler | None  # Allow None if file handler fails
    log_path: str
...
def InitLogger(logfilename: str, debug: bool = False) -> LoggerOptions:
    ...
    logging_level = logging.DEBUG if debug else getattr(logging, os.getenv("LOG_LEVEL", "INFO").upper(), logging.INFO)

    # Create console logger
    try:
        # Force reinitialization if already configured
        ...
        logging.basicConfig(format="%(levelname)s: %(message)s", level=logging_level, force=True)
        logging.warning(f"Unable to set utf-8 encoding for console log, falling back to default: {e}")

    # Create file handler with the same logging level
    try:
        os.makedirs(config_dir, exist_ok=True)
        file_handler = logging.FileHandler(log_path, encoding="utf-8", mode="w")
        formatter = logging.Formatter("%(asctime)s - %(levelname)s - %(message)s")  # More detailed format for file
        file_handler.setFormatter(formatter)
        file_handler.setLevel(logging_level)  # Use the same level as console
...
def CreateArgParser(description: str) -> ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Number of seconds between lines to consider for batching",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="A brief description of the film to give context",
    )
    parser.add_argument(
        "--addrtlmarkers",
        action="store_true",
        help="Add RTL markers to translated lines if they contains primarily right-to-left script",
    )
    parser.add_argument(
        "--includeoriginal",
        action="store_true",
        help="Include the original text in the translated subtitles",
    )
    parser.add_argument(
        "--instruction",
        action="append",
        type=str,
        default=None,
        help="An instruction for the AI translator",
    )
    parser.add_argument(
        "--instructionfile",
        type=str,
        default=None,
        help="Name/path of a file to load instructions from",
    )
    parser.add_argument(
        "--matchpartialwords",
        action="store_true",
        help="Allow substitutions that do not match not on word boundaries",
    )
    parser.add_argument(
        "--maxbatchsize",
        type=int,
        default=None,
        help="Maximum number of lines before starting a new batch is compulsory",
    )
    parser.add_argument(
        "--maxlines",
        type=int,
        default=None,
        help="Maximum number of lines(subtitles) to process in this run",
    )
    parser.add_argument(
        "--maxsummaries",
        type=int,
        default=None,
        help="Maximum number of context summaries to provide with each batch",
    )
    parser.add_argument(
        "--minbatchsize",
        type=int,
        default=None,
        help="Minimum number of lines to consider starting a new batch",
    )
    parser.add_argument(
        "--moviename",
        type=str,
        default=None,
        help="Optionally specify the name of the movie to help the translator",
    )
    parser.add_argument(
        "--name",
        action="append",
        type=str,
        default=None,
        help="A name to use verbatim in the translation",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=None,
        help="Postprocess the subtitles after translation",
    )
    parser.add_argument(
        "--preprocess",
        action="store_true",
        default=None,
        help="Preprocess the subtitles before translation",
    )
    parser.add_argument(
        "--project",
        type=str,
        default=None,
        help="Read or Write project file to working directory",
    )
    parser.add_argument(
        "--ratelimit",
        type=int,
        default=None,
        help="Maximum number of batches per minute to process",
    )
    parser.add_argument(
        "--scenethreshold",
        type=float,
        default=None,
        help="Number of seconds between lines to consider a new scene",
    )
    parser.add_argument(
        "--substitution",
        action="append",
        type=str,
        default=None,
        help="A pair of strings separated by ::, to subsitute in source or translation",
    )
    parser.add_argument(
        "--temperature",
        type=float,
        default=0.0,
        help="A higher temperature increases the random variance of translations.",
    )
    parser.add_argument(
        "--writebackup",
        action="store_true",
        help="Write a backup of the project file when it is loaded (if it exists)",
    )
    return parser
...
def CreateOptions(args: Namespace, provider: str, **kwargs) -> Options:
    ...
    options = {
        "api_key": getattr(args, "apikey", None),  # Handle missing apikey attribute gracefully
    ...
    # Add model if present in args (specific providers add this)

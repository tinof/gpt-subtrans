...
class Instructions:
    ...
    def LoadInstructionsFile(self, filepath: str):
        ...
        for line in lines:
            ...
            elif section_name and (line.strip() or sections.get(section_name)):
        ...
        if filepath:
            ...
            with open(filepath, "w", encoding="utf-8", newline="") as f:
                if self.prompt:
                    f.write("### prompt\n")
                    f.write(self.prompt)
                if self.task_type and self.task_type != DEFAULT_TASK_TYPE:
                    f.write(f"\n\n### task_type\n{self.task_type}")
...
def GetInstructionsFiles():
    ...
    return sorted(
        list(instructions_map.values()),
        key=lambda x: (x.lower() != "instructions.txt", x.lower()),
    )

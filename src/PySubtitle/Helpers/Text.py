...
def LimitTextLength(text: str, max_length: int) -> str:
    ...
    for position, _match in reversed(matches):
...
def RemoveFillerWords(text: str, fillerWords: str | list[str] | regex.Pattern) -> str:
    ...
    def _append_previous_section(
        output: list[str], text: str, previous_start: int, previous_end: int, capitalise_first_letter: bool
    ):

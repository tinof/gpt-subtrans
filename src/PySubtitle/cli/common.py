...
def CreateArgParser(description: str) -> ArgumentParser:
    ...
    parser.add_argument(
        "--batchthreshold", type=float, default=None, help="Number of seconds between lines to consider for batching"
    )
    ...
    parser.add_argument(
        "--matchpartialwords", action="store_true", help="Allow substitutions that do not match not on word boundaries"
    )
    parser.add_argument(
        "--maxbatchsize", type=int, default=None, help="Maximum number of lines before starting a new batch is compulsory"
    )
    parser.add_argument("--maxlines", type=int, default=None, help="Maximum number of lines(subtitles) to process in this run")
    parser.add_argument(
        "--maxsummaries", type=int, default=None, help="Maximum number of context summaries to provide with each batch"
    )
    parser.add_argument(
        "--minbatchsize", type=int, default=None, help="Minimum number of lines to consider starting a new batch"
    )
    parser.add_argument(
        "--moviename", type=str, default=None, help="Optionally specify the name of the movie to help the translator"
    )
    ...
    parser.add_argument(
        "--scenethreshold", type=float, default=None, help="Number of seconds between lines to consider a new scene"
    )
    parser.add_argument(
        "--substitution",
        action="append",
    ...
    parser.add_argument(
        "--temperature", type=float, default=0.0, help="A higher temperature increases the random variance of translations."
    )
    parser.add_argument(
        "--writebackup", action="store_true", help="Write a backup of the project file when it is loaded (if it exists)"
    )
    return parser

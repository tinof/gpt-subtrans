...
class TranslationParser:
    ...
    def GetRegularExpressionPatterns(self, task_type: str = DEFAULT_TASK_TYPE):
        ...
        patterns = [
            regex.compile(pattern.replace(DEFAULT_TASK_TYPE, task_type), regex.MULTILINE)
        ...
        if unmatched:
            self.errors.append(
                UntranslatedLinesError(
                    f"No translation found for {len(unmatched)} lines", lines=unmatched, translation=translation
                )
            )
